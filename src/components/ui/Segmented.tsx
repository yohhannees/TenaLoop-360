...
export default function Segmented({ label, options, value, onChange }: Props) {
  return (
    <div className="grid gap-2">
      ...
      <div
        className="grid min-w-0 gap-2 rounded-[1.5rem] bg-[#0A2318]/8 p-1"
      ...
      >
        {options.map((option) => (
          <button
            ...
            className={cn(
              "h-10 min-w-0 rounded-full border px-2 text-sm font-semibold leading-tight transition sm:px-3",
              option === value
